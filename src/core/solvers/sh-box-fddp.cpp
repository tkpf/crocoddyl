...
#include "crocoddyl/core/solvers/sh-box-fddp.hpp"
...
SolverSHBoxFDDP::SolverSHBoxFDDP(boost::shared_ptr<ShootingProblem> problem)
    : SolverSHFDDP(problem),
...
SolverSHBoxFDDP::~SolverSHBoxFDDP() {}

void SolverSHBoxFDDP::resizeData() {
  START_PROFILER("SolverSHBoxFDDP::resizeData");
  SolverSHFDDP::resizeData();
  ...
  STOP_PROFILER("SolverSHBoxFDDP::resizeData");
}

void SolverSHBoxFDDP::allocateData() {
  SolverSHFDDP::allocateData();
...
void SolverSHBoxFDDP::forwardPass(const double steplength) {
...
const std::vector<Eigen::MatrixXd>& SolverSHBoxFDDP::get_Quu_inv() const {
  return Quu_inv_;
}
