...
#include "crocoddyl/core/solvers/sh-fddp.hpp"
#include "crocoddyl/core/utils/exception.hpp"

namespace crocoddyl {

SolverSHFDDP::SolverSHFDDP(boost::shared_ptr<ShootingProblem> problem)
    : SolverSHDDP(problem), dg_(0), dq_(0), dv_(0), th_acceptnegstep_(2) {}

SolverSHFDDP::~SolverSHFDDP() {}
...
bool SolverSHFDDP::solve(const std::vector<Eigen::VectorXd>& init_xs,
  ...
  START_PROFILER("SolverSHFDDP::solve");
  ...
  for (iter_ = 0; iter_ < maxiter; ++iter_) {
    ...
    if (steplength_ <= th_stepinc_) {
      increaseRegularization();
      if (preg_ == reg_max_) {
        STOP_PROFILER("SolverSHFDDP::solve");
        return false;
      }
    }
    stoppingCriteria();
...
const Eigen::Vector2d& SolverSHFDDP::expectedImprovement() {
  dv_ = 0;
...
void SolverSHFDDP::updateExpectedImprovement() {
  dg_ = 0;
  dq_ = 0;
  const std::size_t T = this->problem_->get_T();
  if (!is_feasible_) {
...
void SolverSHFDDP::forwardPass(const double steplength) {
  if (steplength > 1. || steplength < 0.) {
    throw_pretty("Invalid argument: "
                 << "invalid step length, value is between 0. to 1.");
  }
  START_PROFILER("SolverSHFDDP::forwardPass");
  cost_try_ = 0.;
  ...
  if ((is_feasible_) || (steplength == 1)) {
    for (std::size_t t = 0; t < T; ++t) {
      ...
      if (raiseIfNaN(cost_try_)) {
        STOP_PROFILER("SolverSHFDDP::forwardPass");
...
double SolverSHFDDP::get_th_acceptnegstep() const { return th_acceptnegstep_; }

void SolverSHFDDP::set_th_acceptnegstep(const double th_acceptnegstep) {
