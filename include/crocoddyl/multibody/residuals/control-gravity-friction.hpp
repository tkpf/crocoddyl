...
#ifndef CROCODDYL_MULTIBODY_RESIDUALS_CONTROL_GRAVITY_FRICTION_HPP_
#define CROCODDYL_MULTIBODY_RESIDUALS_CONTROL_GRAVITY_FRICTION_HPP_
...
class ResidualModelControlGravFricTpl : public ResidualModelAbstractTpl<_Scalar> {
 public:
  ...
  typedef ResidualDataControlGravFricTpl<Scalar> Data;
   ...
   */
  ResidualModelControlGravFricTpl(boost::shared_ptr<StateMultibody> state,
                              const VectorXs& vf_coeff,
  ...
  ResidualModelControlGravFricTpl(boost::shared_ptr<StateMultibody> state,
                              const VectorXs& vf_coeff,
                              const VectorXs& cf_coeff);
  virtual ~ResidualModelControlGravFricTpl();
 ...
 private:
  ...
  ArrayXs viscous_friction_coeff_; //!<Viscous friction coefficients
  ArrayXs static_friction_coeff_; //!<Coulomb friction coefficients
  // if we use smooothing function: auto smooth_sign = qdot.array() / (qdot.array().square() + eps).sqrt();
  // Scalar eps_friction_smoothing = 1e-6; //!< Epsilon to smooth sign function // TODO make getter, setter, constructor
  // if we use smoothing function: auto smooth_sign = (k * qdot.array()).tanh();
  Scalar k_friction_smoothing = 10.0;
};

template <typename _Scalar>
struct ResidualDataControlGravFricTpl : public ResidualDataAbstractTpl<_Scalar> {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  ResidualDataControlGravFricTpl(Model<Scalar> *const model,
                             DataCollectorAbstract *const data)
      : Base(model, data) {
...
/* --- Details -------------------------------------------------------------- */
/* --- Details -------------------------------------------------------------- */
#include "crocoddyl/multibody/residuals/control-gravity-friction.hxx"

#endif  // CROCODDYL_MULTIBODY_RESIDUALS_CONTROL_GRAVITY_FRICTION_HPP_
