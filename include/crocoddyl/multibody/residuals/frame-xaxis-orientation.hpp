...
class ResidualModelFrameXAxisOrientationTpl : public ResidualModelAbstractTpl<_Scalar> {
 public:
  ...
  typedef ResidualDataFrameXAxisOrientationTpl<Scalar> Data;
   ...
   */
  ResidualModelFrameXAxisOrientationTpl(boost::shared_ptr<StateMultibody> state,
  ...
  ResidualModelFrameXAxisOrientationTpl(boost::shared_ptr<StateMultibody> state,
                                const pinocchio::FrameIndex id,
                                const Matrix3s& Rref);
  virtual ~ResidualModelFrameXAxisOrientationTpl();
...
template <typename _Scalar>
struct ResidualDataFrameXAxisOrientationTpl : public ResidualDataAbstractTpl<_Scalar> {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  ResidualDataFrameXAxisOrientationTpl(Model<Scalar>* const model,
                               DataCollectorAbstract* const data)
      : Base(model, data), rJf(3, 3), fJf(6, model->get_state()->get_nv()) {
    r.setZero();
    rRf.setIdentity();  // should we rename these to e.g. xaligned_rRf
    rJf.setZero();
    fJf.setZero();
